...
watch(
  store.$state,
  (state) => {
    localStorage.setItem("viflexstate", JSON.stringify(state));
  },
  { deep: true }
);

onMounted(() => {
  const getState = localStorage.getItem("viflexstate");
  const json = JSON.parse(getState ?? "{}");
  if (!Object.values(json)?.length) return;
  ...
  const loadChatwoot = () => {
    ...
    script.onload = () => {
      ...
      w.chatwootSDK.run({
        websiteToken: "9UknYwr6C5RgLEsUUGu2FXTY",
        baseUrl: BASE_URL,
      });
    };
